...
pub(crate) fn before_hook_macro(attr: TokenStream, item: TokenStream) -> TokenStream {
...
pub(crate) fn after_hook_macro(attr: TokenStream, item: TokenStream) -> TokenStream {
    let function_name: Ident = parse_macro_input!(attr as Ident);
    let input_fn: ItemFn = parse_macro_input!(item as ItemFn);
    let vis: &Visibility = &input_fn.vis;
    let sig: &Signature = &input_fn.sig;
